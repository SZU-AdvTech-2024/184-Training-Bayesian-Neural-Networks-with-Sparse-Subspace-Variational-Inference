...
def test(model, device, test_loader, args, return_logit=False):
    ...
    if not return_logit:
        ...
        all_targets = torch.cat(all_targets, dim=0)  # 合并所有目标标签
        ece = cal.get_calibration_error(torch.cat(logit, dim=0).cpu(), all_targets.cpu())
...
def main():
    ...
    parser.add_argument('--static_topo', default=ed('STATIC_TOPO', 0), type=int,
    ...
    # bnn parameter
    parser.add_argument('--use_bnn', default=True, action='store_true')
    ...
    parser.add_argument('--eval_bnn', default=0, type=int,
                        help='if 0, eval normal nn; if 1, eval bnn with mean; if >1, eval bnn with mean and sample eval_bnn times')
    parser.add_argument('--same_noise', default=False, action='store_true')

    parser.add_argument('--drop_criteria', default='SNR_mean_abs', type=str,
                        choices=['mean', 'E_mean_abs', 'snr', 'E_exp_mean_abs', 'SNR_mean_abs', 'SNR_exp_mean_abs'])
    parser.add_argument('--lambda_exp', default=1.0, type=float)
    parser.add_argument('--add_reg_sigma', default=False, action='store_true',
                        help='if true, add regularization term for sigma to prevent zeros')
    ...
    if not args.nowandb:
        wandb.init(project='ssvi', name=args.exp_name, config=args)
    ...
    if args.data == 'cifar10':
        ...
        # 选择干扰类型和强度
        corruption_type = 'gaussian_noise'  # 选择干扰类型
        severity = 3  # 选择干扰强度 [1, 5]

        # 加载 CIFAR-10-C 数据集
        train_data = np.load(os.path.join(cifar10c_path, f'{corruption_type}.npy'))
        train_labels = np.load(os.path.join(cifar10c_path, 'labels.npy'))

        # CIFAR-10-C 没有单独的训练集和测试集，这里分割数据
        train_images, test_images = train_data, train_data
        train_targets, test_targets = train_labels, train_labels

        # 转换为 PyTorch 数据集
        train_dataset = torch.utils.data.TensorDataset(
            torch.tensor(train_images, dtype=torch.float32).permute(0, 3, 1, 2) / 255.0,
            torch.tensor(train_targets, dtype=torch.long)
        )
        test_dataset = torch.utils.data.TensorDataset(
            torch.tensor(test_images, dtype=torch.float32).permute(0, 3, 1, 2) / 255.0,
        ...
        dataset1 = torch.utils.data.TensorDataset(
            torch.stack([transform_train(img) for img, _ in train_dataset]),
            torch.tensor(train_targets, dtype=torch.long)
        )
        dataset2 = torch.utils.data.TensorDataset(
            torch.stack([transform_test(img) for img, _ in test_dataset]),
            torch.tensor(test_targets, dtype=torch.long)
        )
        num_classes = 10
    ...
    model = StoResNet18(num_classes=num_classes, use_bnn=args.use_bnn, prior_mean=args.prior_mean,
                        prior_std=args.prior_std, \
                        posterior_mean_init=args.posterior_mean_init, posterior_std_init=args.posterior_std_init,
    ...
    optimizer = optim.SGD(
        [{'params': [param for name, param in model.named_parameters() if not name.endswith('posterior_std')],
    ...
    if args.dense_allocation is not None:
        ...
        pruner = RigLScheduler(model, optimizer, dense_allocation=args.dense_allocation, alpha=args.alpha,
                               delta=args.delta, static_topo=args.static_topo, T_end=T_end, ignore_linear_layers=False,
                               grad_accumulation_n=args.grad_accumulation_n, args=args)
